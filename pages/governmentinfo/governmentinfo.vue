	...
	<view>
		...
		<view class="mt10 ml10 mr10 bg-white radius8">
			...
			<view class="row jcbetween aicenter topeee f14" style="background: #fbfbfb">
				<view class="call" @click="tapCallService"><text class="rout icon-dianhua1 mr5" />联系客服</view>
		...
		<view class="bg-white mt10 ml10 mr10 radius8">
			<view class="ml10 mr10 pt10 pb10 txtcenter black f15">回收清单</view>
			<view v-if="single.recydetail==null||single.recydetail==''" class="row jcleft aicenter detail bg-white">
				<image :src="host+'/images/routine/kehuishouwu.png'" />
				<view class="ml10">
					<view class="black f15">可回收物</view>
					<view class="f13 mt5">品种及重量待回收员上门确认</view>
				</view>
			</view>
			<view v-for="(item,index) in single.recydetail" :key="index" class="row jcleft aicenter detail bg-white">
				<image :src="host+'/images/routine/'+item.image" :data-idx="index" @error="errImg" />
				<view class="ml10">
					<view class="row jcbetween aicenter black f15">
						<view class="bold">{{item.scrapName}}</view>
						<view class="oper orange"><text class="f16 bold">{{item.totalScore}}</text> 量心币</view>
					</view>
					<view class="row jcleft aicenter mt5 f12">
						<view class="laborange radius6 mr10">{{item.tempScore}} 量心币/kg</view>
						<view class="labblue1 radius6">{{item.amount}} kg</view>
					</view>
					<view class="gray f12 mt5">{{item.scrapNames}}</view>
				</view>
			</view>
			<view v-if="single.state==3" class="txtright topeee ml10 mr10 pt10 pb10 pr5 black f15">
				合计：<text class="orange f16 bold">{{single.totalScore}} </text><text class="orange">量心币</text>
			</view>

		</view>

		<!--订单信息、联系电话、详细地址-->
		<!--<view class="mt10 ml10 mr10 pl10 pr10 btmeee radius8 bg-white">
		  <view class="pt10 pb10 black bold7 f16">物品图片</view>
		  <view class="row jcbetween aicenter pt10">
		    <view class="back">订单编号</view><view class="gray">{{single.serialNum}}</view>
		  ...
		  <view class="row jcbetween aicenter pt10">
		    <view class="back">收货地址</view>
		    <view class="gray">{{single.address}}{{single.room}}</view>
		  </view>
		</view>-->
		...
		<view class="m10 pl15 pr15 btmeee radius8 bg-white">
			...
			<!--订单备注-->
			<view v-if="single.remarks!=null" class="serialinfo">
				<view>订单备注</view>
			...
			getDetailed: function(id) {
				...
				that.$request.postToken("/users/recyorder/findById.do", {
					...
					if (res.data.status === 0) {
						...
						if (single.recydetail != null && single.recydetail.length > 0) {
							single.recydetail.forEach(item => {
								item.tempScore = 0;
								if (item.score.indexOf(";") > 0) { // 判断量心币是否有多个
									let temp = item.score.split(";");
									temp.forEach(t => {
										item.tempScore += Number(t); // 累加量心币
									})
								} else {
									item.tempScore = item.score;
								}
							})
						}
						that.single = single;
