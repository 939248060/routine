	...
	<view>
		<view class="viewRow jcbetween  top black">
			...
			<view class="top-right txtcenter column">
				...
				<view class="dis_block f12 white">累计量心币</view>
			</view>
		</view>
		<view class="scroll">
				...
				<block v-for="(items,index) in list" :key="index">
					...
					<navigator :url="'../scoredetails/scoredetails?item='+encodeURIComponent(JSON.stringify(item.score))" v-for="(item,index) in items.data" :key="index">
						<view class="row jcbetween aicenter scorelist">
							...
							<view>
								<view class="black">{{item.score.item}}</view>
							</view>
							<view class="f18 green" v-if="item.score.score >= 0">+{{item.score.score}}</view>
							<view class="f18 red" v-else-if="item.score.score < 0">{{item.score.score}}</view>
	...
	export default {
		...
		methods: {
			...
			getList: function(curr, rid) {
				...
				that.$request.postToken("/users/score/findPage.do", data).then((res) => {
...
<style>
	...
	.scorelist>view:nth-child(2) {
		width: 62%;
	}

	.scorelist>view:nth-child(3) {
		width: 22%;
		text-align: right;
		padding-right: 8px;
	}
